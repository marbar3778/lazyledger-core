...
func TestABCIResponsesSaveLoad1(t *testing.T) {
	...
	abciResponses.DeliverBlock.Txs = dtxs

	abciResponses.DeliverBlock.Txs[0] = &abci.ResponseDeliverTx{Data: []byte("foo"), Events: nil}
	abciResponses.DeliverBlock.Txs[1] = &abci.ResponseDeliverTx{Data: []byte("bar"), Log: "ok", Events: nil}
	abciResponses.DeliverBlock.ValidatorUpdates = types.TM2PB.NewValidatorUpdate(ed25519.GenPrivKey().PubKey(), 10)
...
func TestABCIResponsesSaveLoad2(t *testing.T) {
	...
	for i, tc := range cases {
		...
		responses := &tmstate.ABCIResponses{
			DeliverBlock: &abci.ResponseDeliverBlock{Txs: tc.added},
		}
	...
	for i, tc := range cases {
		...
		if assert.NoError(err, "%d", i) {
			t.Log(res)
			responses := &tmstate.ABCIResponses{
				DeliverBlock: &abci.ResponseDeliverBlock{Txs: tc.expected},
			}
...
// see https://github.com/lazyledger/lazyledger-core/issues/2718
func TestProposerPriorityDoesNotGetResetToZero(t *testing.T) {
	...
	abciResponses := &tmstate.ABCIResponses{
		DeliverBlock: &abci.ResponseDeliverBlock{ValidatorUpdates: nil},
	}
...
func TestLargeGenesisValidator(t *testing.T) {
	...
	// see: https://github.com/lazyledger/lazyledger-core/issues/2960
	...
	abciResponses := &tmstate.ABCIResponses{
		...
		DeliverBlock: &abci.ResponseDeliverBlock{ValidatorUpdates: []abci.ValidatorUpdate{firstAddedVal}},
	}
	block := makeBlock(oldState, oldState.LastBlockHeight+1)
	blockID := types.BlockID{Hash: block.Hash(), PartSetHeader: block.MakePartSet(testPartSize).Header()}
	updatedState, err := sm.UpdateState(oldState, blockID, &block.Header, abciResponses, validatorUpdates)
	...
	for i := 0; i < 10; i++ {
		...
		abciResponses := &tmstate.ABCIResponses{
			DeliverBlock: &abci.ResponseDeliverBlock{ValidatorUpdates: []abci.ValidatorUpdate{addedVal}},
		}
		block := makeBlock(oldState, oldState.LastBlockHeight+1)
		blockID := types.BlockID{Hash: block.Hash(), PartSetHeader: block.MakePartSet(testPartSize).Header()}
	...
	abciResponses = &tmstate.ABCIResponses{
		DeliverBlock: &abci.ResponseDeliverBlock{ValidatorUpdates: []abci.ValidatorUpdate{removeGenesisVal}},
	}
	block = makeBlock(oldState, oldState.LastBlockHeight+1)
	blockID = types.BlockID{Hash: block.Hash(), PartSetHeader: block.MakePartSet(testPartSize).Header()}
	validatorUpdates, err = types.PB2TM.ValidatorUpdates(abciResponses.DeliverBlock.ValidatorUpdates)
